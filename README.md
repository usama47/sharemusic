...
The host and listeners use the same server-clock estimate and audio engine. Scheduled starts use a dedicated timer. During playback, checks run every 250 ms and ignore drift under 80 ms; larger drift gets gentle speed correction capped at ±2%. Automatic seeking is reserved for drift over 1.5 seconds sustained for two seconds, requires buffered audio at the destination, and is limited to once every eight seconds. Buffering or an in-progress seek suspends automatic correction. Joining and explicit room commands still align the track directly. This favors continuous playback over repeatedly interrupting phones for small timing differences. Browser media precision, speaker/Bluetooth latency, buffering and OS suspension can still create audible differences; acoustic alignment is not guaranteed.
